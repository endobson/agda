...
relatively-prime->gcd {a} {b} rp = record
  ...
  f x xa xb = transport (cong (x div_) (rp x xa xb)) div-refl
...
gcdⁱ->relatively-prime g d nonneg-d da db = divℤ-one->one nonneg-d (GCD.f g d da db)
...
relatively-prime-*' {a@(suc _)} {b = b@(suc _)} {c = c@(suc _)} f-b f-c =
  ...
  ¬prime-div-one (_ , p) p%1 with ≤->≤i (div->≤ p%1 tt)
  ...                           | zero-≤i = 0-is-¬prime p
