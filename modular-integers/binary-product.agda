...
module _ {n1 : Nat} {n2 : Nat} (rp : RelativelyPrime⁰ n1 n2) where
  ...
  private
    ...
    z*2-distrib-*' x1 x2 y1 y2 = eq/ _ _ (congruent-mod n%)
      ...
      d1 = ((lc.x * (y1 * y2)) * lc.y) , *-right ℕ->ℤ-* >=> sym p3
      ...
      d2 = ((lc.y * (x1 * x2)) * lc.x) , *-right ℕ->ℤ-* >=> sym p5
      ...
      d3 : (int (n1 *' n2)) div t6
      d3 = ((lc.y * x1) * (lc.x * y2)) , *-right ℕ->ℤ-* >=> sym p6
      d4 : (int (n1 *' n2)) div t7
      d4 = ((lc.x * y1) * (lc.y * x2)) , *-right ℕ->ℤ-* >=> sym p7
