...
module real.open-interval where
...
ℝ∈Iℚ->Overlap z a b (al , au) (bl , bu) =
  (ℝ-bounds->ℚ< z bl au) , (ℝ-bounds->ℚ< z al bu)
...
ℝ-bounds->Iℚ x l<x x<u =
  Iℚ-cons _ _ (ℝ-bounds->ℚ< x l<x x<u)
...
ℝ∈Iℚ-Pos x 0<x = ∥-map2 handle (x.isUpperOpen-L 0r (ℝ<->L 0<x)) x.Inhabited-U
  where
  module x = Real x
...
private
  ...
  tighter-ε qi1@(Iℚ-cons l1 u1 _) qi2@(Iℚ-cons l2 u2 _) (i⊂-cons l2<l1 u1<u2) = (ε , 0<ε) , f
    ...
    f qi3@(Iℚ-cons l3 u3 _) w≤ε (l1<u3 , l3<u1) = i⊆-cons l2≤l3 u3≤u2
      ...
      -w = diff u3 l3
      l2≤l3 : l2 ≤ l3
      l2≤l3 = subst2 _≤_ diff-step diff-step (+-preserves-≤ (weaken-< l1<u3) -dl≤-w)
        where
        -dl≤-w : (diff l1 l2) ≤ -w
        -dl≤-w = subst2 _≤_ (sym diff-anticommute) (sym diff-anticommute)
                        (minus-flips-≤ (trans-≤ w≤ε min-≤-left))
      u3≤u2 : u3 ≤ u2
      u3≤u2 = subst2 _≤_ diff-step diff-step (+-preserves-≤ (weaken-< l3<u1) w≤du)
        where
        w≤du : w ≤ du
        w≤du = trans-≤ w≤ε min-≤-right

  tighter-ε' : (qi1 qi2 : Iℚ) -> (qi1 i⊂ qi2) ->
               Σ[ ε ∈ ℚ⁺ ] ((qi3 : Iℚ) -> i-width qi3 ≤ ⟨ ε ⟩ -> Touching qi3 qi1 -> qi3 i⊆ qi2)
  tighter-ε' qi1@(Iℚ-cons l1 u1 _) qi2@(Iℚ-cons l2 u2 _) (i⊂-cons l2<l1 u1<u2) = (ε , 0<ε) , f
    where
    dl = (diff l2 l1)
    du = (diff u1 u2)
    ...
    f : (qi3 : Iℚ) -> i-width qi3 ≤ ε -> Touching qi3 qi1 -> qi3 i⊆ qi2
    f qi3@(Iℚ-cons l3 u3 _) w≤ε (l1≤u3 , l3≤u1) = i⊆-cons l2≤l3 u3≤u2
      where
      w = diff l3 u3
      -w = diff u3 l3
...
touching-ℝ∈Iℚs->path :
  (x y : ℝ) ->
  ((qi1 qi2 : Iℚ) -> ℝ∈Iℚ x qi1 -> ℝ∈Iℚ y qi2 -> Touching qi1 qi2) ->
  x == y
touching-ℝ∈Iℚs->path x y f = ℝ∈Iℚ->path x y g
  where
  g : (qi : Iℚ) -> ℝ∈Iℚ x qi -> ℝ∈Iℚ y qi
  g qi x∈qi = unsquash (isProp-ℝ∈Iℚ y qi) (∥-bind handle (tighter-ℝ∈Iℚ x qi x∈qi))
    where
    ...
    handle (qi2 , qi2⊂qi , x∈qi2) = handle2 (tighter-ε' qi2 qi qi2⊂qi)
      where
      handle2 : Σ[ ε ∈ ℚ⁺ ] ((qi3 : Iℚ) -> i-width qi3 ≤ ⟨ ε ⟩ -> Touching qi3 qi2 -> qi3 i⊆ qi) ->
                ∥ ℝ∈Iℚ y qi ∥
      handle2 (ε , h) = ∥-bind handle3 (find-small-ℝ∈Iℚ y ε)
        where
        handle3 : Σ[ qi3 ∈ Iℚ ] (ℝ∈Iℚ y qi3 × i-width qi3 ≤ ⟨ ε ⟩) -> ∥ ℝ∈Iℚ y qi ∥
        handle3 (qi3 , y∈qi3 , w-qi3≤ε) = ∣ ℝ∈Iℚ-⊆ y qi3⊆qi y∈qi3 ∣
          where
          o-qi3-qi2 = sym-Touching qi2 qi3 (f qi2 qi3 x∈qi2 y∈qi3)
          qi3⊆qi = h qi3 w-qi3≤ε o-qi3-qi2
