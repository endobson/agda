...
open import real.real-sequence
...
isProp-ΣisInfiniteSum : ∀ {s}  -> isProp (Σ ℝ (isInfiniteSum s))
isProp-ΣisInfiniteSum = isProp-ΣisLimit

isConvergentSequence : Pred Seq ℓ-one
isConvergentSequence s = Σ ℝ (isLimit s)

isConvergentSeries : Pred Seq ℓ-one
isConvergentSeries s = Σ ℝ (isInfiniteSum s)
...
ℝ∈Iℚ-+ᵉ⁻ x y a@(Iℚ-cons l u l≤u) (L-a , U-a) = ∥-map2 handle L-a U-a
  ...
  handle (l1 , l2 , L-l1 , L-l2 , l1+l2=l) (u1 , u2 , U-u1 , U-u2 , u1+u2=u) =
    (Iℚ-cons l1 u1 (weaken-< (ℝ-bounds->ℚ< x l1 u1 L-l1 U-u1))) ,
    (Iℚ-cons l2 u2 (weaken-< (ℝ-bounds->ℚ< y l2 u2 L-l2 U-u2))) ,
    (L-l1 , U-u1) ,
    (L-l2 , U-u2) ,
...
+-preserves-limit {s1} {s2} {v1} {v2} l1 l2 = close->isLimit (s1 + s2) (v1 + v2) f
  where
...
isLimit-constant-seq x = 
  close->isLimit _ _ (\qi x∈qi -> ∣ 0 , (\_ _ -> x∈qi) ∣)

seq-cons : ℝ -> Seq -> Seq
seq-cons x f zero = x
seq-cons x f (suc n) = f n

isLimit-seq-cons : (x : ℝ) (s : Seq) (v : ℝ) -> isLimit s v -> isLimit (seq-cons x s) v
isLimit-seq-cons x s v l = close->isLimit _ _ f
  where
...
module _
  ...
  module _ (x : ℝ) (0<x : 0# < x) (x<1 : x < 1#) where
    ...
    isLimit-geometric-sequence = close->isLimit _ _ f
      where
      ...
      f qi@(Iℚ-cons l u _) (L-l , U-u) =
        ...
        handle _ (q , q<1 , xU-q) = ∥-bind handle2 (Archimedian-ℚ' 0<u 0<q q<1)
          where
          0<u = U->ℚ< U-u
          0<q = ℝ-bounds->ℚ< x 0# q (ℝ<->L 0# x 0<x) xU-q
...
--cons-single-prefix-isInfiniteSum {s} {v} lim = close->isLimit s2 (v + s zero) f
--  where
--  s2 = partial-sums s
