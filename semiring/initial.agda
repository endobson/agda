...
module _ {ℓD : Level} {D : Type ℓD} {ACM : AdditiveCommMonoid D}
  ...
  ℕ->Semiringʰ : Semiringʰ ℕ->Semiring
  ℕ->Semiringʰ = ℕ->SemiringStr.fʰ C

  ∃!ℕ->Semiring : ∃! (ℕ -> D) Semiringʰ
  ∃!ℕ->Semiring = (ℕ->Semiring , ℕ->Semiringʰ) , isProp-ΣSemiringʰ _
    where
    opaque
      isProp-ΣSemiringʰ : isProp (Σ (ℕ -> D) Semiringʰ)
      isProp-ΣSemiringʰ (f , fʰ) (g , gʰ) = ΣProp-path isProp-Semiringʰ fun-path
        where
  ...
  opaque
    ...
    ℕ->Semiring-preserves-1# : Path D (ℕ->Semiring 1) 1#
    ℕ->Semiring-preserves-1# =
      Semiringʰ.preserves-1# (∃!-prop ∃!ℕ->Semiring)

    ℕ->Semiring-preserves-2# : Path D (ℕ->Semiring 2) 2#
