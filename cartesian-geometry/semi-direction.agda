...
data SameSemiDirection : Rel Direction ℓ-one where
  ...
  same-semi-direction-flipped : {d1 d2 : Direction} -> ⟨ d1 ⟩ == (- ⟨ d2 ⟩) -> SameSemiDirection d1 d2
...
-- Constructors
vector->semi-direction : (v : Vector) -> v # 0# -> SemiDirection
...
sym-SameSemiDirection (same-semi-direction-flipped p) =
  same-semi-direction-flipped (sym minus-double-inverse >=> cong -_ (sym p))


private
  vector->semi-direction-v*' :
    (v : Vector) -> (v#0 : v # 0#) -> (k : ℝ) -> (kv#0 : (k v* v) # 0#) ->
    ...
    module a where
      ...
      abstract
        ...
        handle (inj-l k<0) = eq/ _ _ (same-semi-direction-flipped p)
          ...
          v-kv#0 : (- (k v* v)) # 0#
          v-kv#0 = subst (_# 0#) (sym v-p1) -kv#0

          p1 : normalize-vector ((- k) v* v) -kv#0 == (normalize-vector v v#0)
          p1 = normalize-vector-v*-Pos v v#0 -k 0<-k -kv#0
          p2 : normalize-vector (- (k v* v)) v-kv#0 == - (normalize-vector (k v* v) kv#0)
          p2 = normalize-vector-v- (k v* v) kv#0 v-kv#0
          p3 : normalize-vector (- (k v* v)) v-kv#0 == normalize-vector ((- k) v* v) -kv#0
          p3 = cong2-dep normalize-vector v-p1 (isProp->PathPᵉ (\i -> isProp-#) v-kv#0 -kv#0)

          p : normalize-vector (k v* v) kv#0 == - (normalize-vector v v#0)
          p = sym minus-double-inverse >=> cong -_ (sym p2 >=> p3 >=> p1)
...
opaque
  vector->semi-direction-v* :
    (v1 : Vector) (v1#0 : v1 # 0#) (v2 : Vector) (v2#0 : v2 # 0#) (k : ℝ) ->
    (k v* v1 == v2) ->
  ...
  vector->semi-direction-v* v1 v1#0 v2 v2#0 k path =
    ...
    where
    kv1#0 : (k v* v1) # 0#
    kv1#0 = (subst (_# 0#) (sym path) v2#0)
    path3 : PathP (\i -> path i # 0#) kv1#0 v2#0
