...
module _ {ℓK ℓV : Level} {K : Type ℓK} {S : Semiring K} {R : Ring S}
  ...
  module _ {ℓI : Level} {I : Type ℓI} {{FI : FinSetStr I}} where
    ...
    vector-sum : (I -> V) -> V
    vector-sum vs = finiteMerge CommMonoid-V+ M.isSet-V (I , isFinSet-I) vs


  module _ {ℓI₁ ℓI₂ : Level} {I₁ : Type ℓI₁} {I₂ : Type ℓI₂}
           {{FI₁ : FinSetStr I₁}} {{FI₂ : FinSetStr I₂}}
           where

    vector-sum-convert : (eq : I₂ ≃ I₁) (f : I₁ -> V) ->
                         vector-sum f == vector-sum (f ∘ (eqFun eq))
    vector-sum-convert = finiteMerge-convert CommMonoid-V+ M.isSet-V _ _


    vector-sum-⊎ : (f : (I₁ ⊎ I₂) -> V) ->
                   vector-sum f ==
                   vector-sum (f ∘ inj-l) v+ vector-sum (f ∘ inj-r)
    vector-sum-⊎ = finiteMerge-⊎ CommMonoid-V+ M.isSet-V _ _
