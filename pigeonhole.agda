...
module _ where
  private
    abstract
      ...
      smaller-fun-inj : {n m : Nat} -> (f : Fin (suc n) -> Fin (suc m)) ->
                        (inj-f : isInjective f) -> isInjective (smaller-fun f inj-f)
  ...
  pigeonhole-large : {n m : Nat} -> n > m -> (f : (Fin n) -> (Fin m)) -> ¬(isInjective f)
...
module _ where
  abstract
    ...
    pigeonhole-small {n} {m} lt f =
      ...
      where
      g-inj : {g : Fin m -> Fin n} -> (∀ j -> f (g j) == j) -> isInjective g
