...
ΣFin-suc-eq' n P = (isoToEquiv i)
  ...
  forward ((0 , lt) , p) = inj-l (subst P (ΣProp-path isProp≤ refl) p)
  forward ((suc i , lt) , p) = inj-r ((i , pred-≤ lt) , (subst P (ΣProp-path isProp≤ refl) p))
  ...
  module _ (i : Fin n) (p : (P (suc-fin i))) where
    ...
    fb-r-snd : PathP (\k -> P (p1 (~ k))) (subst P p2 p) p
  ...
  bf-zero lt p = (\i -> path1 (~ i) , path2 (~ i))
    ...
    path2 : PathP (\i -> P (path1 i)) p (subst P path1 p)
...
isFinSetΣ-Σ' {ℓ} {suc n} FB = subst isFinSetΣ (sym (ua (ΣFin-suc-eq' n B))) combined
  where
  B : Fin (suc n) -> Type ℓ
  B = fst ∘ FB
...
module _ {ℓD : Level} {D : Type ℓD} {{S : Semiring D}} where
 ...
 --   finiteSum-Σ'-suc' :
 --     (f : (Σ (Maybe (Fin n)) B) -> D) ->
 --     finiteSum (FinSet-Σ (FinSet-Fin 0) FB) f == S.0#
 --   finiteSum-Σ'-zero f = ?
  ...
  --  finiteSum-Σ' {n = zero} f = finiteSum-Σ'-zero f
  --  finiteSum-Σ' {n = suc n} f = ?


 --                       (A : FinSet ℓ) (B : ⟨ A ⟩ -> FinSet ℓ)  ->
 -- finiteSum-Σ'-zero {D = D} {{S}} A B f = ?


--  finiteSum-Σ'-zero : {ℓ : Level} {D : Type ℓ} {{S : Semiring D}}
--                      (A : FinSet ℓ) (B : ⟨ A ⟩ -> FinSet ℓ) (f : (Σ[ a ∈ ⟨ A ⟩ ] ⟨ B a ⟩) -> D) ->
--                       finiteSum (FinSet-Σ A B) f == finiteSum A (\a -> finiteSum (B a) (\b -> f (a , b)))
--  finiteSum-Σ'-zero {D = D} {{S}} A B f = ?




--(finiteSum (FinSet-Fin 0) (f ∘ inj-l))
--
--                   FiniteSum ((A ⊎ B) , isFinSet-⊎ finA finB) ==
--                   cardnality (A , finA) +' cardnality (B , finB)
--
--  finiteSum-⊎ : {ℓ₁ ℓ₂ : Level} {A : Type ℓ₁} {B : Type ℓ₂}
--                (finA : isFinSet A) (finB : isFinSet B) ->
--                FiniteSum ((A ⊎ B) , isFinSet-⊎ finA finB) ==
--                cardnality (A , finA) +' cardnality (B , finB)
--  finiteSum-⊎ finA finB =
